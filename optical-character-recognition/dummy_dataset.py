...
class ConcatDataset(keras.utils.Sequence):
    ...
    def __init__(
        ...
        self.indexes = tf.range(self.cumulative_sizes[-1])
        self.batch_size = batch_size
        self.shuffle = shuffle
        self.collate_fn = collate_fn
    ...
    def __getitem__(self, index):
        if index < 0:
            if -index > len(self.cumulative_sizes[-1]):
                raise ValueError(
                    "absolute value of index should not exceed dataset length"
                )

            index = len(self.cumulative_sizes[-1]) + index
...
class LmdbDataset(keras.utils.Sequence):
    def __init__(self, root, opt, batch_size=4, shuffle=True, collate_fn=None):
        self.root = root
        self.opt = opt
        self.env = lmdb.open(
            root,
...
class RawDataset(keras.utils.Sequence):
    def __init__(self, root, opt, batch_size=4, shuffle=True, collate_fn=None):
        ...
        self.indexs = tf.range(self.nSamples)
        self.batch_size = batch_size
        self.shuffle = shuffle
        self.collate_fn = collate_fn
    ...
    def __call__(self, batch):
        ...
        if self.keep_ratio_with_pad:
            ...
            image_tensors = tf.concat(
                [tf.expand_dims(t, axis=0) for t in resized_images], axis=0
            )
            ...
            image_tensors = tf.concat(
                [tf.expand_dims(t, axis=0) for t in image_tensors], axis=0
            )
